...
const PurchasePOS: React.FC<PurchasePOSProps> = ({ onClose }) => {
  ...
  const itemsDatabase: Item[] = [
    { id: '1', name: 'Wireless Headphones', code: 'WH001', price: 2000, stock: 45, unit: 'PCS', category: 'Electronics', gst: 18 },
    { id: '2', name: 'Office Chair', code: 'OC002', price: 7500, stock: 8, unit: 'PCS', category: 'Furniture', gst: 12 },
    { id: '3', name: 'Laptop Stand', code: 'LS003', price: 1000, stock: 25, unit: 'PCS', category: 'Accessories', gst: 18 },
    { id: '4', name: 'Wireless Mouse', code: 'WM004', price: 650, stock: 72, unit: 'PCS', category: 'Electronics', gst: 18 },
    { id: '5', name: 'Desk Lamp', code: 'DL005', price: 1500, stock: 15, unit: 'PCS', category: 'Lighting', gst: 12 },
    { id: '6', name: 'USB Cable', code: 'UC006', price: 200, stock: 100, unit: 'PCS', category: 'Accessories', gst: 18 },
    { id: '7', name: 'Monitor Stand', code: 'MS007', price: 1200, stock: 20, unit: 'PCS', category: 'Accessories', gst: 18 },
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      // // Block and open Item Master on typing any alphabet/symbol
      // if (/[^0-9]/.test(event.key) && event.key.length === 1 && !['Backspace', 'Enter'].includes(event.key)) {        
      ...
      if (event.key === 'F3' && items.length > 0) {
        ...
        setTimeout(() => rateInputRef.current?.focus(), 100);
        return;
      }
    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      ...
      <div className="bg-purple-600 text-white p-4 flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-xl font-bold">Purchase POS</h1>
          <span className="text-purple-200">PO: {poNumber}</span>
        </div>
        <div className="flex items-center space-x-4">
          <input
            type="text"
        ...
        <div className="w-1/2 border-r border-gray-200 p-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
              ...
              <div><kbd className="bg-white px-1 rounded">F2</kbd> Save Purchase</div>
              <div><kbd className="bg-white px-1 rounded">F3</kbd> Edit Rate</div>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center text-gray-500">
              <div className="text-center">
                ...
                <p className="text-sm">Search and add items to start purchasing</p>
              </div>
            </div>
          ) : (
            <>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {items.map((item, index) => (
                      <tr
                        key={item.id}
                        className={`border-b ${selectedItemIndex === index ? 'bg-purple-50 border-purple-200' : 'hover:bg-gray-50'}`}
                      >
                        <td className="p-2">
                        ...
                        <td className="p-2 text-center">
                          {editingField === 'rate' && selectedItemIndex === index ? (
                            <input
                              ref={rateInputRef}
                              type="number"
                              value={item.price}
